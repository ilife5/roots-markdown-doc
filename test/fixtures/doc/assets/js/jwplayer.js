if (typeof jwplayer == "undefined") {
			...
			this.setupPlugins = function(w, s, z) {
				...
				for (var v in s.plugins) {
					...
					if (B && s.plugins && s.plugins[p]) {
						...
						u.plugins[x] = y.getNewInstance(w, a.extend({}, s.plugins[p]), q);
						u.length++;
	...
	(function(b) {
		...
		var e = b.embed = function(t) {
				var p = new e.config(t.config),
					i, k, j = p.width,
					l = p.height,
					n = "Error loading player: ",
					h = b.plugins.loadPlugins(t.id, p.plugins);
				p.id = t.id;
				...
				function f() {
					if (a.typeOf(p.playlist) == "array" && p.playlist.length < 2) {
						if (p.playlist.length == 0 || !p.playlist[0].sources || p.playlist[0].sources.length == 0) {
							r();
							return
						}
					}
					if (h.getStatus() == a.loaderstatus.COMPLETE) {
						for (var w = 0; w < p.modes.length; w++) {
							if (p.modes[w].type && e[p.modes[w].type]) {
								var x = p.modes[w].config,
									u = a.extend({}, x ? e.config.addConfig(p, x) : p),
									v = new e[p.modes[w].type](i, p.modes[w], u, h, t);
								if (v.supportsConfig()) {
		...
		var c = h.config = function(j) {
				function m(q, p, o) {
					for (var n = 0; n < q.length; n++) {
						var r = q[n].type;
						if (!q[n].src) {
							q[n].src = o[r] ? o[r] : p + "jwplayer." + r + (r == "flash" ? ".swf" : ".js")
						}
					}
				}
				var l = {
		...
		var e = c.embed.flash = function(l, m, q, k, o) {
				...
				function p(t, u, s) {
					return function(v) {
						try {
							if (s) {
								document.getElementById(o.id + "_wrapper").appendChild(u)
							}
							var x = document.getElementById(o.id).getPluginConfig("display");
				...
				this.embed = function() {
					...
					var E = "#000000",
						A, v = x.wmode ? x.wmode : (x.height && x.height <= 40 ? "transparent" : "opaque"),
						w = ["height", "width", "modes", "events", "primary", "base", "fallback"];
